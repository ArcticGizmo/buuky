...
import { TDatabase } from '@/types/database.type';
...
class SQLiteService implements ISQLiteService {
  ...
  async openDatabase(dbName: string, version: number, readOnly: boolean, decrypt: boolean = false): Promise<SQLiteDBConnection> {
    ...
    if (retCC && isConn) {
      ...
      if (this.platform !== 'web') {
        ...
        const isDbEncrypt = isDevEncrypt && isDbExists ? (await this.sqliteConnection.isDatabaseEncrypted(dbName)).result! : false;
        const isPassphrase = isDevEncrypt ? await this.isSecretStored() : false;
        encrypted =
          (!isDbExists && isDevEncrypt && isPassphrase) ||
  ...
  async setEncryptionPassphrase(passphrase: string): Promise<void> {
    ...
    const isSetPassphrase = await this.isSecretStored();
    if (isSetPassphrase) {
